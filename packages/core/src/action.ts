...
export function addActionRoutingRule<T>(
    controller: object,
    action: string | symbol,
    group: string | symbol,
...
export type ActionOptions =
    & ActionMetadata
    & { routing?: ActionRoutingOptions | ActionRoutingOptions[] };
...
export function Action(
    options: string | ( string | ActionRoutingOptions )[] | ActionOptions
): MethodDecorator {
    return ( target, key: string | symbol ): void => {
        ...
        for( const item of routing ) {
            ...
            addActionRoutingRule( target, key, group, {
                pattern : String( key ),
                ...rest
            } );
        }
