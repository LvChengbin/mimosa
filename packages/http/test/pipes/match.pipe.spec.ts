...
import { ParameterPipeMetadata, Pipe } from '@mimosa/core';
...
describe( 'Match Pipe', () => {
    ...
    describe( 'default exception', () => {
        ...
        it( 'string pattern', async () => {
            ...
            await expect( fn1( 'xstring', context, application, meta ) ).rejects.toThrowMimosaHttpException( defaultExceptionResponse );
            await expect( fn1( 'stringx', context, application, meta ) ).rejects.toThrowMimosaHttpException( defaultExceptionResponse );
            await expect( fn1( 'abc', context, application, meta ) ).rejects.toThrowMimosaHttpException( defaultExceptionResponse );
        } );
        ...
        it( 'regexp pattern', async () => {
            ...
            await expect( fn2( 'abc', context, application, meta ) ).rejects.toThrowMimosaHttpException( defaultExceptionResponse );
            await expect( fn2( 2, context, application, meta ) ).rejects.toThrowMimosaHttpException( defaultExceptionResponse );
        } );
        ...
        it( 'mixed patterns', async () => {
            ...
            await expect( fn3( 'abc', context, application, meta ) ).rejects.toThrowMimosaHttpException( defaultExceptionResponse );
            await expect( fn3( 2, context, application, meta ) ).rejects.toThrowMimosaHttpException( defaultExceptionResponse );
        } );

        it( 'correct error message while metadata.property is not empty', async () => {
            await expect( fn1( 'abc', context, application, meta ) ).rejects.toThrowMimosaHttpException( {
                status : 400,
                error : statuses.message[ 400 ],
                message : [ 'id should match "string"' ]
            } );

            await expect( fn2( 'abc', context, application, meta ) ).rejects.toThrowMimosaHttpException( {
                status : 400,
                error : statuses.message[ 400 ],
                message : [ `id should match "${/regexp/.toString()}"` ]
            } );
        ...
        it( 'correct error message while metadata.property is empty', async () => {
            await expect( fn1( 'a', context, application, createParameterPipeMetadata() ) ).rejects.toThrowMimosaHttpException( {
                status : 400,
                error : statuses.message[ 400 ],
