...
describe( 'Required Pipe', () => {
    ...
    describe( 'default exception', () => {
        ...
        it( 'should have thrown an error if the given value is null', async () => {
            await expect( fn( null, context, application, meta ) ).rejects.toThrowMimosaHttpException();
        } );

        it( 'should have thrown an error if the given value is undefined', async () => {
            await expect( fn( undefined, context, application, meta ) ).rejects.toThrowMimosaHttpException();
        } );

        it( 'should have thrown exception if the given value is an empty string', async () => {
            await expect( fn( '', context, application, meta ) ).rejects.toThrowMimosaHttpException();
        } );

        it( 'should have thrown HttpException with correct response', async () => {
            return expect( fn( undefined, context, application, meta ) ).rejects.toThrowMimosaHttpException( {
                status : 400,
        ...
        it( 'should thrown HttpException with correct message if metadata.data.property is empty', async () => {
            return expect( fn( undefined, context, application, createParameterPipeMetadata() ) ).rejects.toThrowMimosaHttpException( {
                status : 400,
                error : statuses.message[ 400 ],
    ...
    describe( 'custom exception', () => {
        ...
        it( 'should use the options for creating HttpException', async () => {
            ...
            return expect( Required( args )( undefined, context, application, meta ) ).rejects.toThrowMimosaHttpException( args );
        } );
    } );
} );
