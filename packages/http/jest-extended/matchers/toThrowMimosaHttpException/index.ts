 ...
 *
 * File: toThrowMimosaHttpException/index.ts
...
const passMessage = ( received: any, expected: any ): ( () => string ) => (): string => {
    return matcherHint( '.toThrowMimosaHttpException', 'function', 'type' ) +
        '\n\n' +
...
const failMessage = ( received: any, expected: any ): ( () => string ) => (): string => {
    return matcherHint( '.not.toThrowMimosaHttpException', 'function', 'type' ) +
        '\n\n' +
        'Expected to throw:\n' +
        `  ${printExpected( expected?.toJSON?.() ?? expected )}\n` +
        'Thrown:\n' +
 ...
 * @example
 * ```ts
 * expect( () => { throw new HttpException( 400 ) } ).toThrowMimosaHttpException()
 * ```
 *
 ...
 */
export default function toThrowMimosaHttpException(
    ...
    /**
     * expect( Promise.reject( new HttpException( 400 ) ) ).rejects.toThrowMimosaHttpException();
     */
    if( this.promise ) {
        exception = callback;
    } else {
