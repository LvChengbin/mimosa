...
import { createInterceptorParameterSync } from '@mimosa/method-interceptor';
...
export class Injection {
    ...
    constructor( options: InjectionOptions = {} ) {
        ...
        options.providers && this.#collateProviders( this.#setProvider( options.providers, true ), true );
    }
    ...
    #collateClassProvider( entry: InjectionEntry<ClassProvider>, token: InjectionToken ): void {
        ...
        Reflect.getMetadata( 'design:paramtypes', useClass )?.forEach( ( paramtype: InjectionToken, i: number ) => {
            ...
            if( !this.registeredProviders.has( t ) ) {
                // While the dependency is not in the provider set, throw `UnknownDependencyException` if the param is not optional
    ...
    #instantiateClassProvider<T = unknown>(
        ...
        const {
            constructorParameters,
            injectedProperties
        } = Reflect.getMetadata( INJECT_DEPENDENCIES_METADATA_KEY, useClass ) as ClassProviderDependenciesMetadata;
        ...
        Object.keys( injectedProperties ).forEach( ( key: string | symbol ) => {
            ...
            instance[ key ] ??= token instanceof EmptyDependency ? undefined : this.instantiate( token, cacheMap );
        } );
     ...
     *
     * @param tokens -
     * @param deeply - while setting an `Entrance` provider, it doesn't need to traverse it's dependencies deeply
     */
    ...
    instantiate<T = any>(
        ...
        const entry = this.dependencies.get( token );
        if( !entry ) throw new UndefinedDependencyException( token );
